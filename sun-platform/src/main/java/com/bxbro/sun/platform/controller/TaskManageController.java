...
public class TaskManageController {
    ...
    public BaseResult<Long> upsertTask(@Validated TaskManageForm form) {
        UpsertTaskRequest upsertTaskRequest = new UpsertTaskRequest();
        BeanUtil.copyProperties(form, upsertTaskRequest);
        return taskManageService.upsertTask(upsertTaskRequest);
    }

    @ApiOperation("删除任务")
    @DeleteMapping("/v1/{id}")
    public BaseResult<Void> deleteTask(@PathVariable("id") Long id) {
    ...
    @ApiOperation("查询任务列表")
    @GetMapping("/v1/list")
    public BaseResult<TaskListDataVO> queryTaskList(TaskManageQuery query) {
        TaskListRequest taskListRequest = new TaskListRequest();
        BeanUtil.copyProperties(query, taskListRequest);
